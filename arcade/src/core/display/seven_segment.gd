...
## 表示を更新する。
##
## 桁数を超える値は**頭打ち**にする(3 桁なら 1234 は 999)。下位桁だけを出す
## ラップアラウンドにはしない。1000 枚が 000 と出ると、プレイヤーには
## 「メダルが消えた」としか読めない。999 で止まっていれば「振り切れている」と読める。
## 嘘をつくより振り切れたと分かるほうがましなので頭打ちを選んでいる。
##
## 負の値は 0 に丸める。計数器なので負の表示は存在しない。
